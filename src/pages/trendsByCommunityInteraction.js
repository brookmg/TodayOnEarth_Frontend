...
const PostsByCommunityInteraction = (props) => {
  ...
  const variables = {
    ...
    communityInteractions, // needed to refresh query when it changes
  }
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: 'column' }}>
            ...
            <div>
              <div style={{ textAlign: 'left' }}>
                <FormCheckbox
                  toggle
                  small
                  checked={isRelativeInteraction}
                  onChange={handleRelativeInteractionChange}>
        ...
        <span style={{ flex: 1, alignSelf: 'center' }}>
          ...
          <span>
            {
              Object.keys(communityInteractions).map(
                e => <FormCheckbox
                  inline
                  key={e}
                  checked={!!communityInteractions[e]}
                  onChange={ev => handleCommunityInteractionChange(ev, e)}
                >
                  {e}
                </FormCheckbox>
              )
